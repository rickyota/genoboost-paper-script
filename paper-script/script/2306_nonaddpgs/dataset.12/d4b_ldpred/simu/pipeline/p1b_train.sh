#!/bin/bash

#PJM -L rscgrp=gg57,node=1
#PJM -L elapse=8:00:00
##PJM -L elapse=24:00:00
#PJM -j
#PJM -g gg57
#PJM --bulk

#$ -S /bin/bash
#$ -cwd
#$ -j y
#$ -pe smp 28
#$ -q all.q
##$ -l hostname=a02|a03|z01|z02|g01|g02
#$ -l hostname=z01|z02|g01|g02

set -eux

kind="simu"
program="ldpred"
dataname="dataset.12"

hostname
pwd
echo "Start $(readlink -f $0)"

if [ -v SGE_TASK_ID ]; then
    job_id=$((SGE_TASK_ID - 1))
    threads=$NSLOTS
elif [ -v PJM_BULKNUM ]; then
    job_id=$PJM_BULKNUM
    ## FIXME
    threads=64
else
    job_id=$3
    threads=16
fi

if [ -v PJM_BULKNUM ]; then
    # load from env variables
    :
else
    #dataname="$1"
    #fgenot_ref="$2"
    simu_type="$1" # male, female, both
    phes="$2"
    #cvn="$3"
fi

rhos="0.1 0.01 0.001"

cvi=0
phei=${job_id}
phes_ar=(${phes})
phe=${phes_ar[$phei]}

ddata="./data/${dataname}/"
dresult="./result/nonaddpgs/${dataname}/${program}/${kind}/${simu_type}/"
mkdir -p $dresult

dout="${dresult}/${phe}/train.cv${cvi}/raw/"
mkdir -p $dout

fgenot_ref="./data/1kg/eur.qc/1kg.eur.qc"
fss="./result/nonaddpgs/${dataname}/assoc/${kind}/${simu_type}/ukb_imp.cv${cvi}.${phe}.glm.logistic.hybrid.ss"
fcv="${ddata}ukb_imp.cv"

#if [ $sex == "female" ]; then
#samplen=$(awk -v cv=${cvi} '($2==0 && $(3+cv) == "tr"){ print $1 }' $fcv | awk 'END{print NR}')
#elif [ $sex == "both" ]; then
samplen=$(awk -v cv=${cvi} '($(3+cv) == "tr"){ print $1 }' $fcv | awk 'END{print NR}')
#fi

echo $samplen

flog="./log/nonaddpgs/${dataname}/${program}/${kind}/${simu_type}/${phe}/train.cv${cvi}/train.$(date +'%y%m%d-%H%M%S').log"
mkdir -p "$(dirname $flog)"

bash ./script/lib/${program}/train.sh \
    $dout $fss $fgenot_ref $samplen "$rhos" $threads |& tee $flog
