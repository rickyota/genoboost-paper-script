#!/bin/bash

#$ -S /bin/bash
#$ -cwd
#$ -j y
#$ -pe smp 28
#$ -q all.q
#$ -l hostname=a02|a03|z01|z02|g01|g02
##$ -l hostname=a02|a03

set -eux

kind="ex-chr6-gwasmethods"
program="ldpred"
dataname="dataset.12"

hostname
pwd
echo "Start $(readlink -f $0)"

if [ -v SGE_TASK_ID ]; then
    job_id=$((SGE_TASK_ID - 1))
    threads=$NSLOTS
else
    job_id=$5
    threads=16
fi


#fgenot_ref="$2"
sex="$1" # male, female, both
phes="$2"
cvn="$3"

rhos="0.1 0.01 0.001"

cvi=$((job_id % cvn))
phei=$((job_id / cvn))
phes_ar=(${phes})
phe=${phes_ar[$phei]}


ddata="./data/${dataname}/"
dresult="./result/nonaddpgs/${dataname}/${program}/${kind}/"
mkdir -p $dresult

dout="${dresult}/${phe}/train.cv${cvi}/raw/"
mkdir -p $dout

#fgenot_ref="./data/1kg/eur.qc/1kg.eur.qc"
fgenot_ref="/nfs/data06/ricky/data/1000G/1000G_EUR_Phase3_plink_one_rsformat/1000G.EUR.QC"

#fss="./result/nonaddpgs/${dataname}/assoc/ukb_imp.cv${cvi}.${phe}.glm.logistic.hybrid.ss"
dtmp="${ddata}/tmp/${program}/${kind}/"
fss="${dtmp}/${phe}.cv${cvi}.ss.LDpred"
fcv="${ddata}ukb_imp.cv"

if [ $sex == "female" ]; then
    samplen=$(awk -v cv=${cvi} '($2==0 && $(3+cv) == "tr"){ print $1 }' $fcv | awk 'END{print NR}')
elif [ $sex == "both" ]; then
    samplen=$(awk -v cv=${cvi} '($(3+cv) == "tr"){ print $1 }' $fcv | awk 'END{print NR}')
fi

echo $samplen


flog="./log/nonaddpgs/${dataname}/${program}/${kind}/${phe}/train.cv${cvi}/train.$(date +'%y%m%d-%H%M%S').log"
mkdir -p "$(dirname $flog)"

# same as train.sh?
bash ./script/lib/${program}/train-gwasmethods.sh \
    $dout $fss $fgenot_ref $samplen "$rhos" $threads |& tee $flog
