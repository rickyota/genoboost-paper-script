...
def format_wgt(dout, dwgt, fgenot, genot_format, sex, is_nsnv):
    # args=sys.argv
    # dout=args[1]
    ...
    # ls snpnet.n*
    if is_nsnv:
        g_fscore = str(dwgt) + '/snpboost_n-*.raw'
        # g_fscore = str(dwgt) + '/snpnet.n*.raw'
    else:
        g_fscore = str(dwgt) + '/snpboost.raw'
    ...
    for f in fscores:
        ...
        if is_nsnv:
            ...
            nsnvs = len(wgt_snv)
            fout = dout / ('snpboost_n-' + str(nsnvs) + '.wgt')
            # fout=f.replace('.raw','.wgt')
        else:
            fout = dout / ('snpboost.wgt')
        print("fout", fout)
        ...
        if is_nsnv:
            fout_cov = dout / ('snpboost_n-' + str(nsnvs) + '.wgtcov')
            # fout_cov = dout / ('snpnet_n-' + str(nsnvs) + '.wgtcov')
        else:
            fout_cov = dout / ('snpboost.wgtcov')
...
def main():
    ...
    format_wgt(pathlib.Path(args.dout), pathlib.Path(args.dwgt), pathlib.Path(args.fgenot), args.genot_format, args.sex, args.is_nsnv)
